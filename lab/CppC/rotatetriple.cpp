...
public:
    ...
    static RotateList<A,B,C> * create2(A a, B b, C c, int i){
      if (i<=0) return nullptr;
      Triple<A,B,C>* x= new Triple<A,B,C>(a,b,c);
      Triple<B,C,A>* x1= new Triple<B,C,A>(b,c,a);
      Triple<C,A,B>* x2 = new Triple<C,A,B>(c,a,b);
      RotateList<A,B,C>* r = new RotateList<A,B,C>(x, nullptr);
      RotateList<A,B,C>* temp = r; // point to r
      RotateList<B,C,A>* temp1;
      RotateList<C,A,B>* temp2;
      int count = 1;
      while (count < i) {
        if (count % 3 == 1) {
          temp1 = new RotateList<B,C,A>(x1, nullptr);
          temp->next = temp1;
        }
        if (count % 3 == 2) {
          temp2 = new RotateList<C,A,B>(x2, nullptr);
          temp1->next = temp2;
        }
        if (count % 3 == 0) {
          temp = new RotateList<A,B,C>(x, nullptr);
          temp2->next = temp;
        }
        count++;
      }
